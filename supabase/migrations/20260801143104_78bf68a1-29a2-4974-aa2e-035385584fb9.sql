...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'recharge_l1', COALESCE((SELECT SUM(rc.amount) FROM public.recharges rc WHERE rc.status = 'Success' AND rc.user_id = ANY(l1)), 0),
    'recharge_l2', COALESCE((SELECT SUM(rc.amount) FROM public.recharges rc WHERE rc.status = 'Success' AND rc.user_id = ANY(l2)), 0),
    'recharge_l3', COALESCE((SELECT SUM(rc.amount) FROM public.recharges rc WHERE rc.status = 'Success' AND rc.user_id = ANY(l3)), 0),
    'commission_l1', COALESCE((SELECT SUM(t.amount) FROM public.transactions t WHERE t.user_id = p.id AND t.title = 'Level 1 team commission'), 0),
    'commission_l2', COALESCE((SELECT SUM(t.amount) FROM public.transactions t WHERE t.user_id = p.id AND t.title = 'Level 2 team commission'), 0),
    'commission_l3', COALESCE((SELECT SUM(t.amount) FROM public.transactions t WHERE t.user_id = p.id AND t.title = 'Level 3 team commission'), 0),
